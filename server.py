...
app.config['MONGO_URI'] = os.environ.get('MONGO_URI', 'mongodb://localhost:27017/mulapal_db')
    ...
    try:
        ...
        if status in ('COMPLETED','PAID','SUCCESS'):
            ...
            if not pay or pay.get('status') != 'completed':
                ...
                users_collection.update_one({'_id': pay['user_id']}, {'$inc': {'balance': float(pay['amount'])}})
                transactions_collection.insert_one({
                    'user_id': pay['user_id'], 'type': 'deposit', 'amount': float(pay['amount']), 'currency': pay.get('currency','KES'),
...
def initiate_pesapal_payment(current_user):
    try:
        ...
        payment_data = {
            ...
            'callback_url': app.config['PESAPAL_CALLBACK_URL'],
            'notification_id': app.config['PESAPAL_CALLBACK_URL'],
            'billing_address': {
                        ...
                        if response.status_code == 200:
                            ...
                            if payment_status == 'COMPLETED':
                                ...
                                if user:
                                    ...
                                    <p>Your deposit of <strong>{format_amount(amount, current_user.get('currency', 'USD'))}</strong> has been successfully processed.</p>
                                    <p>Transaction ID: <strong>{order_merchant_reference}</strong></p>
                                    <p>Your new balance: <strong>{format_amount(user['balance'] + amount, current_user.get('currency', 'USD'))}</strong></p>
                                    <br>
