...
<head>
    ...
    <title>Games Earn | Matrix Platform</title>
    ...
    <style>
        ...
        .claim-btn { background: #059669; color: white; padding: 0.75rem 1.5rem; border-radius: 8px; border: none; cursor: pointer; }
    </style>
</head>
<body>
    <div class="max-w-4xl mx-auto p-6">
        <div class="flex items-center gap-3 mb-6">
            <i class="fas fa-gamepad text-3xl text-emerald-600"></i>
            <h1 class="text-3xl font-bold">Games Earn</h1>
        </div>
        <p class="text-gray-600 mb-6">Complete gaming tasks and earn rewards.</p>
    ...
    <script>
        ...
        async function loadTasks() {
            try {
                const response = await fetch(`${API_BASE}/tasks?category=games&page=1&limit=10`, {
                ...
                if (data.status === 'success') {
                    ...
                    if (tasks.length === 0) {
                        document.getElementById('tasksContainer').innerHTML = `
                            <div class="text-center py-8">
                                ...
                                <h3 class="text-xl font-semibold mb-2">No Gaming Tasks Available</h3>
                                <p class="text-gray-600">Check back later for new gaming challenges.</p>
                            </div>
                        `;
                        return;
                    }
                    ...
                    document.getElementById('tasksContainer').innerHTML = tasks.map(task => `
                        <div class="task-card">
                            ...
                            <p class="text-emerald-600 font-medium mb-3">Reward: ${task.priceDisplay}</p>
