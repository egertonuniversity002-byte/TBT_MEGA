...
<head>
  ...
  <title>Team Level 1 - Inactive</title>
  <style>
...
<body>
  <div class="container">
    <h2>Level 1 - Inactive Referrals</h2>
    <p class="muted">Direct referrals not meeting the active criteria.</p>
...
<script>
  ...
  async function load() {
    try {
      const res = await fetch(`${API_BASE}/team/list?level=1&status=inactive`, { headers: { 'Authorization': `Bearer ${token}` }});
